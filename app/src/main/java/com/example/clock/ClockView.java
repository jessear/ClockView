...
public class ClockView extends View{
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.translate(canvas.getWidth()/2-mRadius,mRadius); //�ı仭��λ��
		canvas.drawCircle(mRadius, mRadius, mRadius, paint);//��Ȧ
		...
		canvas.drawLine(mRadius, mRadius,
				...
				mRadius-(float) (mHourLen*Math.cos(((hours+minutes/60f)*30)*Math.PI/180)),paint);//ʱ��ָ��
		
		paint.setColor(Color.BLUE);
		canvas.drawLine(mRadius, mRadius,
				mRadius+(float) (mMinuteLen*Math.sin(minutes*6*Math.PI/180)),
				mRadius-(float) (mMinuteLen*Math.cos(minutes*6*Math.PI/180)),paint);//����ָ��
		...
		canvas.drawLine(
				...
				mRadius-(float)((mSecondLen-mDistance)*Math.cos(seconds*6*Math.PI/180)),paint);//����ָ��
		...
		for(int i=1;i<=12;i++){
			canvas.drawText(i+"", 
					...
					(float) (mRadius+mTextSize/2-(mRadius-mTextSize*1.5f)*Math.cos(30*i*Math.PI/180)), paint);//�̶�
		}
